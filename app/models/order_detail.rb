class OrderDetail < ApplicationRecord
  ...
  scope :income_monthy, -> {group_by_month(:created_at).sum("quantity * price")}

  scope :income_yearly, -> {group_by_year(:created_at).sum("quantity * price")}

  scope :income_daily, -> {group_by_day(:created_at).sum("quantity * price")}
end
